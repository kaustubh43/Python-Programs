...
def reset_password(email: str):
    ...
    if user:
        msg = Message(subject=f'Forgot your planetary api password?',
...
def add_planet():
    ...
    if test:
        ...
        db.session.commit()
        return jsonify(message=f'You added {planet_name} in your db'), 201
...
def update_planet():
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message=f"You updated a planet {request.form['planet_name']}"), 202
    else:
        return jsonify(message='Planet you are trying to update does not exist'), 404
...
def remove_planet(planet_id: int):
    ...
    if planet:
        ...
        db.session.commit()
        return jsonify(message=f'You have deleted {planet.planet_name}')
    else:
        return jsonify(message='Planet that you requested is not found')
